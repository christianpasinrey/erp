...
import type { Contact, OrganizationOption } from '@/types';
...
const form = useForm({
    ...
    organization_id: props.contact.organization_id ?? ('' as string | number),
...
<template>
    <TenantLayout :breadcrumbs="[{ title: t('contacts.contacts'), href: '/contacts' }, { title: contact.display_name }, { title: t('common.edit') }]">
        <div class="p-6 max-w-3xl">
            <h1 class="text-2xl font-bold mb-6">{{ t('contacts.editContact') }}</h1>

            <form @submit.prevent="submit">
                <Card>
                    <CardHeader>
                        <CardTitle>{{ form.type === 'person' ? t('contacts.person') : t('contacts.organization') }}</CardTitle>
                    </CardHeader>
                ...
                <div class="flex items-center gap-3 mt-6">
                    ...
                    <Button variant="outline" as-child>
                        <Link :href="`/contacts/${contact.id}`">{{ t('common.cancel') }}</Link>
                    </Button>
                </div>
            </form>
        </div>
